...
/**
 * This widget permit you to display a SuggestBox with a link in the bottom with which the user can add gracefully a new
 * suggest box and so on. The list of suggest box has only one oracle {@code MultiWordSuggestOracle}. Attually the
 * oracle is populated only once when the user create the widget so ALL the data are fetch immediately.
 * 
 * @param <Type>
 */
public abstract class SuggestList<Type extends Serializable> extends Composite
{
    ...
    /**
     * Metodo di utilità interna. Si occupa di ridisegnare la tabella che mostra i suggest box
     */
